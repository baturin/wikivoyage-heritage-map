...
function initMultiplePages(pageNames)
{
    ...
    var long = getUrlParameter('long');
    var zoom = getUrlParameter('zoom');
    var sequential = getUrlParameter('sequential');

    var executionFunction = (sequential === 'true' || sequential === '1') ? runSequence : runAsync;
...
function initMonumentMap() {
    var pageNames = getUrlParameter('name').split(',');

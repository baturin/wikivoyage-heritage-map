...
function getMonumentImagePopupContent(monument) {
    ...
    popupContentsElement.append('&nbsp;');
    popupContentsElement.append(imageMagnifyLinkElement);
...
function loadMonuments(pageName, onSuccess, loadAddress)
{
    ...
    $.ajax({
        ...
        data: $.param({
            ...
            items: 'monuments',
            fields: composeFieldsRequest(fields),
            filter: 'without-wikivoyage-boundary,able-to-display-on-map'
        })
    }).done(function (result) {
        onSuccess(result.data.monuments);
    });
...
function createOuterPolygon(polygon)
{
    var world = [
        L.latLng([90, 360]),
        L.latLng([90, -180]),
...
function showDistrictBoundaries(map, boundaries)
{
    if (boundaries && boundaries.length === 1) {
        var outerPolygon = createOuterPolygon(boundaries[0]);
        var polygonStyle = {
            weight: 0,
            fillColor: '#555555',
        };

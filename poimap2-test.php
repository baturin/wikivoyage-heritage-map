...
<html>
<!-- 
  PoiMap2 - Version 2012-04-15

  Author:
  http://de.wikivoyage.org/wiki/User:Mey2008
     
  Contributors:
  http://en.wikivoyage.org/wiki/User:Torty3
  http://en.wikivoyage.org/wiki/Benutzer:Nicolas1981
  ...
  2014-04-07: grey zoom display, - blue circles
  2014-03-29: + varible decimal digits for coordinates popup 
  2014-03-28: php notes & warnings
  2014-03-21: + layer Boundaries
  2014-03-16: - layer Wikivoyage, + layer Mapnik b&w, - markersAttribution, minZoom=2, maxZoom=18
  2014-03-10: addTO error, empty gpx error
  2014-02-23: + DMS to DEC, fixed auto zoom
  2014-02-12: + type=health
  ToDo: 
  2014-03-31: gpx layer for internet explorer
  2014-04-07: marker cluster light
-->
...
$content = str_ireplace(array('[[', ']]', '| ', ' |', '= ', ' =', '===', '&', '{{Marker', '{{Listing', '{{vCard', '?lang=', '@', '{{Poi', '=listing' ), array('', '', '|', '|', '=', '=', 'XXX', '%26', '{{listing', '{{listing', '{{listing', 'XxxxxX', 'X', '{{poi', '=' ),  $content);
...
// convert to {{listing|
$content = preg_replace("/{{(go|see|do|buy|eat|drink|sleep|fun|vicinity|health)/", "{{listing|type=$1", $content);
 
// strip unwanted templates
$content = preg_replace("/{{(?!poi|listing|mapframe|mapmask|geo)(.|\s)*?}}/im", "", $content);
...
// automatic numbering for some versions
  if ( $lang == "el" || $lang == "en" || $lang == "es" || $lang == "fr" || $lang == "he" || $lang == "it" || $lang == "pt" || $lang == "ru" || $lang == "uk" || $lang == "zh") {
    $p[$z + $i] = $nr;
...
// checking types
$types = array("**h2**", "black", "blue", "buy", "do", "drink", "eat", "error", "forestgreen", "fun", "go", "gold", "health", "lime", "listing", "maroon", "mediumaquamarin", "other", "red", "see", "silver", "sleep", "steelblue", "view", "vicinity", "health");
$i = 1;
while ($i <= $max){
...
// alert("stop for testing"); // *** TEST ***
  ...
  var fmlng=e.latlng.lng.toFixed(map.getZoom() * 0.25 + 0.5);
    popup
    .setLatLng(e.latlng)
    .setContent('You clicked the map at <br> lat=' + fmlat + ' | long=' + fmlng)
    .openOn(map);
  }
  ...
  // Layer "POI"
  var markers = new L.MarkerClusterGroup({showCoverageOnHover: false, maxClusterRadius: 13,
  ...
  for (var i = 0; i < addressPoints.length; i++) {
    ...
    if (a[0] >= jslat-maxdist && a[0] <= jslat+maxdist && a[1] >= jslon-(maxdist*1.5) && a[1] <= jslon+(maxdist*1.5)) {
      ...
      marker.bindPopup(content + title + '">' + a[2] + '</a><br />').openPopup();
